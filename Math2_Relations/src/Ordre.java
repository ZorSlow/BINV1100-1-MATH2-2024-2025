...
public class Ordre extends RelationAbstraite {
	...
	public Ordre(Relation r) {
		...
		Relation reClone = r.clone();
		//TODO
		if (!reClone.reflexive())
			reClone.cloReflex();
		if (!reClone.transitive())
			reClone.cloTrans();
		if (!reClone.symetrique())
			reClone.antisymetrique();
		if (!reClone.antisymetrique())
			throw new IllegalArgumentException();
		this.couples = reClone;
	}
